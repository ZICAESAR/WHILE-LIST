...
import os # IMPOST
import sys # IMPOST
import pymongo # IMPOST
import subprocess # IMPOST
import time # IMPOST
import gratient # IMPOST
import webbrowser # IMPOST
import ctypes # IMPOST
from psutil import process_iter # IMPOST
import threading # IMPOST
import winreg # IMPOST
import psutil # IMPOST
import platform # IMPOST
import requests # IMPOST
import getmac# IMPOST
import ssl # IMPOST
import socket # IMPOST
import OpenSSL # IMPOST
import difflib # IMPOST

# WHILELIST BY Zlr Sa#5607
class SSLPinner:
...
ctypes.windll.kernel32.SetConsoleTitleW("WHITE-LIST-MANAGER")
hwid = str(str(subprocess.check_output('wmic csproduct get uuid')).strip().replace(r"\r", "").split(r"\n")[1].strip())
Mongo = ""
client = pymongo.MongoClient(Mongo) ##
database = client["test"] ##
_Cdatabase = database["stupids"] ##
if os.path.exists("config.json"):
    openjosn = open("config.json", "r")
    xvxjson = json.loads(openjosn.read())
    showkey = xvxjson["key"]
    showid = xvxjson['id']
...
def sendhwid_database():
    openjosnxx = open("config.json", "r")
    xvxjsonxx = json.loads(openjosnxx.read())
    showkeyxx = xvxjsonxx["key"]
    showidxx = xvxjsonxx['id']
    hwidbacksend = str(str(subprocess.check_output('wmic csproduct get uuid')).strip().replace(r"\r", "").split(r"\n")[1].strip())
    result__ = _Cdatabase.find_one({'key': str(showkeyxx),'userid': str(showidxx)})
    object_C = {'key': str(showkeyxx),'userid': str(showidxx)} 
    newvu = { "$set": { "hwid": str(hwidbacksend) } }
    if not str(result__) == "None":
        _Cdatabase.update_one(object_C, newvu)
    else:
        print("    [!] WRONG-HWID >")
...
def hwidchecker():
    if os.path.exists("hwidchecker.json"):
        openjSn = open("hwidchecker.json", "r")
        jsonxvc = json.loads(openjSn.read())
        fhwid = jsonxvc["hwid"]
        fonetendhwid = str(str(subprocess.check_output('wmic csproduct get uuid')).strip().replace(r"\r", "").split(r"\n")[1].strip())
        os.system('cls' if os.name == 'nt' else 'clear')
        wait(5)
        if fhwid == fonetendhwid:
    ...
    LOADER > LOADING
            ...
            """))
            wait(2)
            result___ = _Cdatabase.find_one({'key': str(showkey),'userid': str(showid),'hwid': str(fonetendhwid)})
            if not str(result___) == "None":
                WORK()
        else:
            os.system('cls' if os.name == 'nt' else 'clear')
            print(gratient.blue(f"""
    ...
    WRONG-HWID... 
        ...
        open("hwidchecker.json", "x")
        hwidbackend = str(str(subprocess.check_output('wmic csproduct get uuid')).strip().replace(r"\r", "").split(r"\n")[1].strip())
        if os.path.exists("hwidchecker.json"):
            fop = open("hwidchecker.json", "w")
            os.system('cls' if os.name == 'nt' else 'clear')
            fop.write(json.dumps({'hwid': hwidbackend}, separators=(',', ':')))
            fop.close()
            wait(5)
            openjosnx = open("config.json", "r")
            xvxjsonx = json.loads(openjosnx.read())
            showkeyx = xvxjsonx["key"]
            showidx = xvxjsonx['id']
            openjSn = open("hwidchecker.json", "r")
            jsonxvc = json.loads(openjSn.read())
            fhwid = jsonxvc["hwid"]
            if fhwid == hwidbackend:
                print(gratient.blue(f"""
    ...
    LOADER > LOADING
                ...
                """))
                wait(2)
                result___ = _Cdatabase.find_one({'key': str(showkeyx),'userid': str(showidx),'hwid': str(hwidbackend)})
                if not str(result___) == "None":
                    WORK()
...
def topx_option():
    ...
    """))
    optionmode = input('    |    MODE > ') 
    if optionmode == "1":
        if os.path.exists("config.json"):
            fopr1 = open("config.json", "r")
            jsonv = json.loads(fopr1.read())
            fkey = jsonv["key"]
            fid = jsonv['id']
            wait(1)
            result_ = _Cdatabase.find_one({'key': str(fkey)})
            result__ = _Cdatabase.find_one({'key': str(fkey),'userid': str(fid)})
            _str = str(result_)
            __str = str(result__)
            os.system('cls' if os.name == 'nt' else 'clear')
            wait(1)
            show(gratient.blue('LOAD-SAVE >'))
            wait(2)
            if _str == "None" and __str == "None":
                top_skip()
                show(gratient.blue(' <  NOT FOUND [ KEY ] > '))
                show(gratient.blue(' <  NOT FOUND [ ID  ] > '))
                wait(2)
                top_option()
            else:
                if _str == "None": ## ถ้ามันไม่ถูกต้อง และ หาไม่เจอ
                    top_skip()
                    show(gratient.blue(' <  NOT FOUND [ KEY ] > '))
                    wait(2)
                    top_option()
                    ...
                    if __str == "None":
                        ...
                        wait(2)
                        top_option()
                    else: ## ถ้ามันถูกต้อง และ หาเจอ
                        top_skip()
                        print(gratient.blue("    FOUND > "+str(fkey))) ## ใส่ function ที่ต้องการได้เลย
                        print(gratient.blue("    FOUND > "+str(fid))) ## ใส่ function ที่ต้องการได้เลย
                        wait(1)
                        hwidchecker()
            ...
            open("config.json", "x")
            if os.path.exists("config.json"):
                fopw2 = open("config.json", "w")
                top()
                fkey = input((('    KEY > '))) 
                fid = input((('    DISCORD-ID > ')))
                fopw2.write(json.dumps({'key': fkey, 'id': fid}, separators=(',', ':')))
                fopw2.close()
                wait(1)
                result_ = _Cdatabase.find_one({'key': str(fkey)})
                result__ = _Cdatabase.find_one({'userid': str(fid)})
                _str = str(result_)
                __str = str(result__)
                os.system('cls' if os.name == 'nt' else 'clear')
                wait(1)
                show(gratient.blue('SAVE >'))
                wait(2)
                if _str == "None" and __str == "None":
                    top_skip()
                    show(gratient.blue(' <  NOT FOUND [ KEY ] > '))
                    ...
                    if _str == "None": ## ถ้ามันไม่ถูกต้อง และ หาไม่เจอ
                        ...
                        if __str == "None": ## ถ้ามันไม่ถูกต้อง และ หาไม่เจอ
                            top_skip()
                            show(gratient.blue(' <  NOT FOUND [ ID  ] > '))
                            wait(5)
                            top_option()
    ...
    if optionmode == "2":
        webbrowser.open("https://discord.gg/hee")
        top_option()
    if optionmode == "3":
        sys.exit()
...
# if os.path.exists("config.json"):
    ...
    # else:
        ...
        # if os.path.exists("config.json"):
            ...
            # top()
            # ask1 = input((('    KEY > '))) 
            # ask2 = input((('    DISCORD-ID > ')))
            # fopw2.write(json.dumps({'key': ask1, 'id': ask2}, separators=(',', ':')))
            # fopw2.close()
